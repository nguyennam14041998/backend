...
/**
 * REST controller for managing {@link com.api.backend.domain.DanToc}.
 */
@RestController
@RequestMapping("/api")
public class DanTocResource {
     ...
     *
     * @param danTocDTO the danTocDTO to create.
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new danTocDTO, or with status {@code 400 (Bad Request)} if the danToc has already an ID.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("/dan-tocs")
    public ResponseEntity<DanTocDTO> createDanToc(@Valid @RequestBody DanTocDTO danTocDTO) throws URISyntaxException {
        log.debug("REST request to save DanToc : {}", danTocDTO);
        if (danTocDTO.getId() != null) {
            throw new BadRequestAlertException("A new danToc cannot already have an ID", ENTITY_NAME, "idexists");
        }
        DanTocDTO result = danTocService.save(danTocDTO);
     ...
     *
     * @param danTocDTO the danTocDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated danTocDTO,
     * or with status {@code 400 (Bad Request)} if the danTocDTO is not valid,
     * or with status {@code 500 (Internal Server Error)} if the danTocDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/dan-tocs")
    public ResponseEntity<DanTocDTO> updateDanToc(@Valid @RequestBody DanTocDTO danTocDTO) throws URISyntaxException {
        log.debug("REST request to update DanToc : {}", danTocDTO);
        if (danTocDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        DanTocDTO result = danTocService.save(danTocDTO);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, danTocDTO.getId().toString()))
            .body(result);
    }
     ...
     */
    @GetMapping("/dan-tocs")
    public ResponseEntity<List<DanTocDTO>> getAllDanTocs(DanTocCriteria criteria, Pageable pageable) {
        log.debug("REST request to get DanTocs by criteria: {}", criteria);
        Page<DanTocDTO> page = danTocQueryService.findByCriteria(criteria, pageable);
    ...
    */
     ...
     *
     * @param id the id of the danTocDTO to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the danTocDTO, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/dan-tocs/{id}")
    public ResponseEntity<DanTocDTO> getDanToc(@PathVariable Long id) {
        log.debug("REST request to get DanToc : {}", id);
        Optional<DanTocDTO> danTocDTO = danTocService.findOne(id);
        return ResponseUtil.wrapOrNotFound(danTocDTO);
    }

    /**
     * {@code DELETE  /dan-tocs/:id} : delete the "id" danToc.
     *
     * @param id the id of the danTocDTO to delete.
